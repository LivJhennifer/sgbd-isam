...
class ISAM:
    ...
    def delete(self, key):
        """
        Localiza e remove um registro. 
        Se uma página de overflow ficar vazia, ela é removida da cadeia.
        """
        leaf, path = self._find_leaf(key)
        print(f"\n[REMOÇÃO] Chave {key} | Caminho: {' → '.join(path)}")

        # tenta remover da folha primária
        for i, record in enumerate(leaf.records):
            if record[0] == key:
                leaf.records.pop(i)
                self.metrics["remocoes"] += 1
                print(f"  → Chave {key} removida da folha primária {leaf.name}.")
                return True

        # tenta remover das páginas de overflow
        prev_node = leaf
        current_ov = leaf.overflow_head
        
        while current_ov is not None:
            for i, record in enumerate(current_ov.records):
                if record[0] == key:
                    current_ov.records.pop(i)
                    self.metrics["remocoes"] += 1
                    print(f"  → Chave {key} removida de uma página de overflow de {leaf.name}.")
                    
                    # Se a página de overflow ficou vazia, libera ela
                    if len(current_ov.records) == 0:
                        if prev_node == leaf:
                            leaf.overflow_head = current_ov.next
                        else:
                            prev_node.next = current_ov.next
                        self.metrics["overflow_liberadas"] += 1
                        print(f"  → Página de overflow vazia liberada.")
                    
                    return True
            
            prev_node = current_ov
            current_ov = current_ov.next
